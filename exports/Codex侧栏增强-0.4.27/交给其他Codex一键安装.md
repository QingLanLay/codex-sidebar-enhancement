# 给其他 Codex 的一键安装说明（0.4.27）

将本说明和 `Codex侧栏增强-0.4.27-Windows.zip` 一起交给目标电脑上的 Codex。请让 Codex 在二者所在目录读取本说明并执行安装；不要只复制 ZIP 内的单个文件。
...
$zip = Get-ChildItem -LiteralPath (Get-Location).Path -Filter 'Codex侧栏增强-0.4.27-Windows.zip' -File | Select-Object -First 1
if (-not $zip) { throw '当前目录没有 Codex侧栏增强-0.4.27-Windows.zip；请先把完整安装包放到这里。' }
$extract = Join-Path $env:TEMP ('CodexSidebarEnhancement-0.4.27-' + [guid]::NewGuid().ToString('N'))
Expand-Archive -LiteralPath $zip.FullName -DestinationPath $extract
$bundle = Join-Path $extract 'Codex侧栏增强-0.4.27'
