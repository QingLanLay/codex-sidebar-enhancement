(() => {
  ...
  const KEY='__sidebarToggleProbe', VERSION='0.4.27.20', STORAGE='diy-sidebar-enhancement-v2';
  ...
  function apply(){
    ...
    status.textContent=`已开启 · ${stats.projects} 个项目 · 最近活动排序`;
  ...
  const settingsButton=document.createElement('button');settingsButton.type='button';settingsButton.textContent='设置';settingsButton.style.cssText='padding:3px 7px;cursor:pointer;border:1px solid #64748b;border-radius:6px;background:#18212f;color:#fff;font:11px system-ui;';
  panel.append(settingsButton);document.body.append(panel);
  function updateSettingsButton(){settingsButton.title='侧栏增强设置\n项目排序 '+(enabled?'开':'关')+' · 快捷栏 '+(quickBarEnabled?'开':'关')+' · 右侧栏 '+(rightPanelEnabled?'开':'关')+' · 容量自动继续 '+(capacityRetryEnabled?'开':'关');}
  ...
  function settingRow(label,checked){const row=document.createElement('label');row.style.cssText='display:flex;align-items:center;justify-content:space-between;gap:12px;cursor:pointer;';const text=document.createElement('span');text.textContent=label;const input=document.createElement('input');input.type='checkbox';input.className='diy-settings-checkbox';input.checked=checked;row.append(text,input);return {row,input};}
  settingsButton.onclick=()=>{
    const existing=document.getElementById('diy-usage-settings');if(existing){existing.remove();return;}
    ...
    const title=document.createElement('strong');title.textContent='侧栏增强设置';
    const projectSort=settingRow('项目按最近会话活动排序',enabled),quick=settingRow('输入框下方快捷会话栏',quickBarEnabled),right=settingRow('右侧当前会话历史提问',rightPanelEnabled),allowReturn=settingRow('移除的会话再次活动后回到快捷栏',allowDismissedReturn),capacityRetry=settingRow('模型容量错误每 10 秒自动继续',capacityRetryEnabled);
    const separator=document.createElement('div');separator.style.cssText='height:1px;background:#ffffff18;margin:2px 0;';
    const address=document.createElement('input');address.type='url';address.required=true;address.setAttribute('aria-label','额度接口地址');address.placeholder='额度接口地址';address.value=window.__diyUsageConfig?.url||'';
    const key=document.createElement('input');key.type='password';key.autocomplete='new-password';key.setAttribute('aria-label','额度密钥');key.placeholder=window.__diyUsageConfig?.configured?'额度密钥已保存，留空保持不变':'输入额度密钥';
    ...
    const message=document.createElement('div');message.textContent='开关即时保存；额度地址和密钥仅保存在本机。';message.style.opacity='.7';
    const actions=document.createElement('div');actions.style.cssText='display:flex;gap:6px;flex-wrap:wrap;';
    const save=document.createElement('button');save.type='submit';save.textContent='保存额度设置';
    const restoreDismissed=document.createElement('button');restoreDismissed.type='button';restoreDismissed.textContent='恢复已移除会话';restoreDismissed.onclick=()=>{dismissedRecent.clear();dismissedAt={};try{localStorage.setItem('diy-dismissed-recent-conversations','[]');localStorage.setItem('diy-dismissed-recent-at','{}');}catch{}recentSignature='';message.textContent='已恢复全部手动移除的会话。';mountPanel();};
    const close=document.createElement('button');close.type='button';close.textContent='关闭';close.onclick=()=>form.remove();
    ...
    actions.append(save,restoreDismissed,close);form.append(title,projectSort.row,quick.row,right.row,allowReturn.row,capacityRetry.row,separator,address,key,message,actions);document.body.append(form);
    form.onsubmit=e=>{e.preventDefault();let url;try{url=new URL(address.value.trim());if(!['http:','https:'].includes(url.protocol))throw Error();}catch{message.textContent='请输入有效的 HTTP 或 HTTPS 地址。';return;}
      if(!key.value.trim()&&!window.__diyUsageConfig?.configured){message.textContent='请输入密钥。';return;}
      window.__diyUsageConfigRequest={id:Date.now(),url:url.origin,key:key.value.trim()};key.value='';message.textContent='正在保存并刷新…';save.disabled=true;
    };
  };
  updateSettingsButton();
  const recentBar=document.createElement('div');recentBar.id='diy-recent-conversations';
  recentBar.setAttribute('aria-label','快捷会话');recentBar.style.cssText='position:relative;display:block;width:100%;max-width:100%;box-sizing:border-box;margin-top:5px;padding-bottom:3px;overflow-x:auto;overflow-y:hidden;overscroll-behavior-x:contain;scrollbar-width:thin;';
  ...
  function updateRecentButton(button,item,index){
    ...
    const nextTitle=item.project+' · '+item.title+(item.running?' · 正在运行':item.completed?(item.viewed?' · 已完成 · 已查看':' · 已完成 · 未查看'):'')+(item.active?' · 当前活动会话':'');if(button.title!==nextTitle)button.title=nextTitle;
    ...
    if(item.running&&!indicator){indicator=document.createElement('span');indicator.className='diy-runtime-indicator';indicator.setAttribute('role','status');indicator.setAttribute('aria-label','正在运行');top.append(indicator);}
    else if(!item.running)indicator?.remove();
  }
  ...
  function showUsageQuota(step=false){
    ...
    usageBox.textContent=quotaLabel(quota,usageIndex,quotas)+' 剩余 '+remaining.toFixed(0)+'%\n额度 '+(usageIndex+1)+' / '+usageCount;usageBox.style.whiteSpace='pre-line';
    usageBox.title='账号共享剩余额度（非现金余额）\n当前额度：'+quotaLabel(quota,usageIndex,quotas)+'\n剩余：'+remaining.toFixed(2)+'%\n重置：'+(quota.resetAt?new Date(quota.resetAt).toLocaleString():'—')+'\n点击切换下一项'+(data.failed?'\n刷新失败，当前为上次数据':'\n每 30 秒刷新');
    return true;
  }
  ...
  function mountUsageBox(){
    ...
    if(usageHidden){usageBox.textContent=data?'额度已隐藏 · 点击显示':'额度隐藏 · 点击显示';usageBox.style.whiteSpace='normal';usageBox.title='点击显示额度；按住 Shift 点击可切换额度项';usageBox.setAttribute('aria-label','额度已隐藏，点击显示');return;}
    if(!data){usageBox.textContent='额度未配置';usageBox.title='额度接口尚未配置';return;}
    if(!showUsageQuota()){usageBox.textContent='额度暂不可用';usageCount=0;}
    usageBox.style.opacity=data.failed?'.55':'1';
    usageBox.title='账号共享剩余额度（非现金余额）\n点击隐藏额度；按住 Shift 点击切换额度项'+(data.failed?'\n刷新失败，当前为上次数据':'\n每 30 秒刷新');usageBox.setAttribute('aria-label','额度显示中，点击隐藏');
  }
  const historyPanel=document.createElement('section');historyPanel.id='diy-input-history';historyPanel.setAttribute('aria-label','当前会话历史提问');historyPanel.style.cssText='display:flex;flex:1 1 auto;min-height:0;height:100%;flex-direction:column;color:inherit;font:12px system-ui;overflow:hidden;';
  ...
  const historyTitle=document.createElement('span');historyTitle.textContent='当前会话 · 历史提问';historyTitle.style.cssText='min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
  const historyCollapseButton=document.createElement('button');historyCollapseButton.type='button';historyCollapseButton.setAttribute('aria-label','收起历史提问栏');historyCollapseButton.title='收起历史提问栏';historyCollapseButton.style.cssText='display:flex;flex:0 0 28px;align-items:center;justify-content:center;width:28px;height:28px;padding:0;border:1px solid color-mix(in srgb,currentColor 14%,transparent);border-radius:6px;background:transparent;color:inherit;opacity:.8;cursor:pointer;';
  ...
  const historyExpandButton=document.createElement('button');historyExpandButton.id='diy-input-history-expand';historyExpandButton.type='button';historyExpandButton.textContent='历史';historyExpandButton.title='展开右侧历史提问';historyExpandButton.setAttribute('aria-label','展开右侧历史提问');historyExpandButton.style.cssText='position:fixed;right:8px;bottom:8px;z-index:2147483647;padding:3px 7px;border:1px solid #64748b;border-radius:6px;background:#18212f;color:#fff;font:11px system-ui;cursor:pointer;';historyExpandButton.onclick=()=>setHistoryCollapsed(false,true);
  ...
  function mountInputHistory(){
    ...
    if(!items.length){historyTitle.textContent=current&&source!==current?'当前会话 · 正在读取历史…':'当前会话 · 暂无历史提问';const empty=document.createElement('div');empty.textContent=current&&source!==current?'正在读取当前会话历史…':'当前会话暂无历史提问';empty.style.cssText='padding:8px;opacity:.55;';historyBody.append(empty);return;}
    historyTitle.textContent='当前会话 · '+items.length+' 条历史提问';
    for(const item of items){const card=document.createElement('div');card.draggable=true;card.tabIndex=0;card.dataset.historyTurnId=item.id;card.setAttribute('role','button');card.setAttribute('aria-label','定位历史提问：'+item.title);card.title='单击定位当前会话中的这条提问；拖到输入框可引用提问和 Codex 回复';card.style.cssText='position:relative;box-sizing:border-box;flex:0 0 148px;width:100%;height:148px;max-height:148px;min-width:0;padding:9px 10px;overflow:hidden;contain:paint;border:1px solid '+(item.current?'#3b82f6':'color-mix(in srgb,currentColor 15%,transparent)')+';border-radius:9px;background:'+(item.current?'color-mix(in srgb,#3b82f6 9%,var(--color-background))':'color-mix(in srgb,currentColor 3%,transparent)')+';box-shadow:0 1px 3px rgba(0,0,0,.12);cursor:pointer;user-select:none;transition:background-color .12s,border-color .12s,box-shadow .12s;';
      const meta=document.createElement('div');meta.style.cssText='display:flex;align-items:center;gap:6px;margin-bottom:6px;';const title=document.createElement('strong');title.textContent=item.title;title.style.cssText='min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;';meta.append(title);if(item.current){const badge=document.createElement('span');badge.textContent='当前';badge.style.cssText='flex:0 0 auto;padding:1px 5px;border-radius:8px;background:#3b82f6;color:white;font-size:9px;line-height:1.5;';meta.append(badge);}
      const userLabel=document.createElement('div');userLabel.textContent='你';userLabel.style.cssText='font-size:10px;font-weight:600;opacity:.65;';
      const userExcerpt=document.createElement('div');userExcerpt.textContent=(item.user||'暂无可预览的用户输入').replace(/\s+/g,' ').trim();userExcerpt.style.cssText='display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;max-width:100%;max-height:2.84em;overflow:hidden;line-height:1.42;overflow-wrap:anywhere;word-break:break-word;white-space:normal;';
  ...
  async function nativeMenuWithRemove(row,item){
    ...
    const nativeItems=convert(items);nativeItems.push({id:'diy-remove-separator',type:'separator'},{id:'diy-new-project-conversation',label:'新建项目会话',enabled:!!item.projectId&&(typeof window.electronBridge?.sendMessageFromView==='function'||!!projectNewChatButton(item))},{id:'diy-remove-quick-conversation',label:'从快捷栏移除',enabled:true});
    document.dispatchEvent(new PointerEvent('pointercancel'));
    const selected=await window.electronBridge.showContextMenu(nativeItems);
    if(selected?.id==='diy-new-project-conversation'){try{await createProjectConversation(item);}catch{recentBar.title='无法打开该项目的新会话，请从原生项目栏创建。';}}
  ...
  recentProjectsSection.setAttribute('aria-label','最近项目');recentProjectsSection.style.cssText='display:flex;flex-direction:column;margin:4px 0 6px;';
  const recentProjectsTitle=document.createElement('div');recentProjectsTitle.textContent='最近项目';recentProjectsTitle.style.cssText='padding:5px 8px;opacity:.6;font:500 13px system-ui;';
    ...
    for(const item of items){
      ...
      if(opened){
        ...
        if(source){
          ...
          row.append(copy);
        }else{const hint=document.createElement('div');hint.textContent='请先展开项目所在分区';hint.style.cssText='padding:4px 12px;opacity:.6;font:inherit';row.append(hint);}
      }
  ...
  async function openRecent(item){
    ...
    catch{recentBar.title='当前客户端导航接口不可用，请从原生侧栏打开会话。';}
  }
  ...
  const historyContainer=document.createElement('aside');historyContainer.id='diy-input-history-panel';historyContainer.setAttribute('aria-label','当前会话历史提问');historyContainer.style.cssText='position:relative;display:flex;min-width:0;height:100%;overflow:hidden;border-left:1px solid color-mix(in srgb,currentColor 14%,transparent);background:var(--color-background);color:inherit;font:inherit;';
  ...
  const historyResizeHandle=document.createElement('div');historyResizeHandle.setAttribute('role','separator');historyResizeHandle.setAttribute('aria-label','拖拽调整右侧会话栏宽度');historyResizeHandle.setAttribute('aria-orientation','vertical');historyResizeHandle.tabIndex=0;historyResizeHandle.style.cssText='position:absolute;left:0;top:0;bottom:0;z-index:3;width:7px;transform:translateX(-3px);cursor:col-resize;touch-action:none;background:transparent;transition:background-color .12s;';
  ...
  function setHistoryCollapsed(value,persist=false){
    ...
    historyCollapseButton.title=historyCollapsed?'展开历史提问栏':'收起历史提问栏';historyCollapseButton.setAttribute('aria-label',historyCollapseButton.title);historyCollapseButton.setAttribute('aria-expanded',String(!historyCollapsed));
  ...
  function syncOutputPanelRightBridge(){
    ...
    const outputNode=panelOpen?[...document.querySelectorAll('[class~="group/summary-panel"]')].find(node=>(node.textContent||'').includes('输出内容')&&node.getClientRects().length>0&&!node.closest('[aria-hidden="true"]')):null;
