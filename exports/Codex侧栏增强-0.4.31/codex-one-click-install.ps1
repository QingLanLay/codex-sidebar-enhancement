...
try {
    ...
    $appEntry = @($manifest.Package.Applications.Application | Where-Object { $_.Id -eq 'App' } | Select-Object -First 1)
    if (-not $appEntry) { throw '当前 Codex 程序包缺少可启动的 App 入口。' }
