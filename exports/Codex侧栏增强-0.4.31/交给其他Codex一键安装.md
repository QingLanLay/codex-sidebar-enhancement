# 给其他 Codex 的一键安装说明（0.4.31）

将本说明和 `Codex侧栏增强-0.4.31-Windows.zip` 一起交给目标电脑上的 Codex。请让 Codex 在二者所在目录读取本说明并执行安装；不要只复制 ZIP 内的单个文件。
...
$zip = Get-ChildItem -LiteralPath (Get-Location).Path -Filter 'Codex侧栏增强-0.4.31-Windows.zip' -File | Select-Object -First 1
if (-not $zip) { throw '当前目录没有 Codex侧栏增强-0.4.31-Windows.zip；请先把完整安装包放到这里。' }
$extract = Join-Path $env:TEMP ('CodexSidebarEnhancement-0.4.31-' + [guid]::NewGuid().ToString('N'))
Expand-Archive -LiteralPath $zip.FullName -DestinationPath $extract
$bundle = Join-Path $extract 'Codex侧栏增强-0.4.31'
...
- Windows 版 Codex：**26.924.1866.0 或更新版本**
- Node.js：**22 或更新版本**
...
Provide this guide together with `Codex侧栏增强-0.4.31-Windows.zip`. Keep both files in the same folder and ask Codex to run the following PowerShell from that folder. Administrator privileges are not required.

```powershell
$ErrorActionPreference = 'Stop'
$zip = Get-ChildItem -LiteralPath (Get-Location).Path -Filter 'Codex侧栏增强-0.4.31-Windows.zip' -File | Select-Object -First 1
if (-not $zip) { throw 'The Codex Sidebar Enhancement 0.4.31 Windows ZIP was not found in this folder.' }
$extract = Join-Path $env:TEMP ('CodexSidebarEnhancement-0.4.31-' + [guid]::NewGuid().ToString('N'))
Expand-Archive -LiteralPath $zip.FullName -DestinationPath $extract
$bundle = Join-Path $extract 'Codex侧栏增强-0.4.31'
$installer = Join-Path $bundle 'codex-one-click-install.ps1'
...
Compatibility: Windows Codex **26.924.1866.0 or later** and Node.js **22 or later**. The installer validates the installed package manifest and Node.js version; do not bypass those checks. A future Codex UI change may still require a newer enhancement script.
