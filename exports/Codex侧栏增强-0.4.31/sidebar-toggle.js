(() => {
  ...
  const KEY='__sidebarToggleProbe', VERSION='0.4.31.0', STORAGE='diy-sidebar-enhancement-v2', UI_LANGUAGE_KEY='diy-sidebar-ui-language';
  ...
  function retryMainSurface(){return document.querySelector('main[data-app-shell-main-surface],main,[data-app-shell-focus-area="main"]')||document.body;}
  ...
  function queueCapacityRetryScan(delay=120){if(!capacityRetryEnabled||capacityRetryScanTimer!==null)return;capacityRetryScanTimer=setTimeout(()=>{capacityRetryScanTimer=null;capacityRetryTick();},delay);}
  function setCapacityRetryEnabled(value){
    ...
    if(capacityRetryEnabled){
      ...
      capacityRetryObserver=new MutationObserver(()=>queueCapacityRetryScan());
  ...
  settingsButton.onclick=()=>{
    ...
    const refreshLabels=()=>{title.textContent=L('侧栏增强设置');projectSort.text.textContent=L('项目按最近会话活动排序');quick.text.textContent=L('输入框下方快捷会话栏');right.text.textContent=L('右侧当前会话历史提问');allowReturn.text.textContent=L('移除的会话再次活动后回到快捷栏');capacityRetry.text.textContent=L('失败会话每 10 秒自动重试');language.text.textContent=L('界面语言');language.select.setAttribute('aria-label',L('界面语言'));for(const option of language.select.options)option.textContent=L(option.dataset.label);address.setAttribute('aria-label',L('额度接口地址'));address.placeholder=L('额度接口地址');key.setAttribute('aria-label',L('额度密钥'));key.placeholder=window.__diyUsageConfig?.configured?L('额度密钥已保存，留空保持不变'):L('输入额度密钥');message.textContent=L(messageKey);save.textContent=L('保存额度设置');restoreDismissed.textContent=L('恢复已移除会话');close.textContent=L('关闭');};
